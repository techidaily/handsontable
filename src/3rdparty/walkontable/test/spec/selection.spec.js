describe('Walkontable.Selection', () => {
  ...
  it('should draw border on two overlays if they are overlapping (scrollable viewport)', function() {
    ...
    expect(pathInTopOverlay.attr('d')).toBe('M 107.5 129.5 107.5 106.5 8.5 106.5 8.5 129.5');

    const pathInLeftOverlay = spec().$wrapper.find('.ht_clone_left svg path').eq(0);
    expect(pathInLeftOverlay.attr('d')).toBe('M 57.5 106.5 8.5 106.5 8.5 152.5 57.5 152.5');

    const pathInTopLeftCornerOverlay = spec().$wrapper.find('.ht_clone_top_left_corner svg path').eq(0);
    expect(pathInTopLeftCornerOverlay.attr('d')).toBe('M 57.5 106.5 8.5 106.5 8.5 129.5');
